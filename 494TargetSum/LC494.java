class Solution {
    ...
    public void dfs(int[] nums, int i, int sums, int target) {
        if (i == nums.length) {
            if (sums == target) {
                count++;
            }
        } else {
            dfs(nums, i + 1, sums + nums[i], target);
            dfs(nums, i + 1, sums - nums[i], target);
...
class Solution {
    ...
    public int dfs(int[] nums, int i, int sums, int target, int[][] memo) {
        if (i == nums.length) {
            if (sums == target)
                return 1;
            else 
            ...
            if (memo[i][sums + 1000] != Integer.MIN_VALUE) {
                return memo[i][sums + 1000];
            }
            int add = dfs(nums, i + 1, sums + nums[i], target,memo);
            int subtract = dfs(nums, i + 1, sums - nums[i], target,memo);
            memo[i][sums + 1000]= add + subtract;
            return memo[i][sums+1000];
                
        }
    }
}
// TC: O(l*N) array size is l,
// SC: O(l*N)
...
class Solution {
    ...
    public int dfs(int[] nums, int i, int sums, int target, HashMap<Pair<Integer,Integer>,Integer> memo) {
        if (memo.containsKey(new Pair<Integer,Integer>(i,sums))) {
            return memo.get(new Pair<Integer, Integer>(i,sums));
        }
        // base case
        if (i == nums.length) {
            if (sums == target)
        ...
        // decision
            int add = dfs(nums, i + 1, sums + nums[i], target,memo);
            int subtract = dfs(nums, i + 1, sums - nums[i], target,memo);
            memo.put(new Pair<Integer, Integer>(i,sums), add + subtract);
